...
class Rocket:
	...
	def __init__(self, *args):
		...
		self.__time = np.arange(0, self.__motor.getBurnTime(), 1e-4)
	...
	def getCOM(self, t):
		...
		return -1/mass*(initMass*initCOM - (rocketLength - motorHeight/2)*
				 (initMotorMass - motorMass))
	...
	def plot(self):
		...
		ax1.set_title('Center of mass of rocket during burn phase of %1.1f' % burnTime)

		ax2 = plt.subplot(212, xlabel='time [s]', ylabel='[kg]')
		ax2.plot(time, mass, label='mass(t)', lw=2, c='b')
		ax2.set_title('Mass of rocket during burn phase of %1.1f' % burnTime)
		plt.show()
		# Moment of inertia
		Ixx = np.array([self.getInertiaMatrix(t)[0][0] for t in time])
		Iyy = np.array([self.getInertiaMatrix(t)[1][1] for t in time])
		# Izz = Iyy, don't need to create another list for this
		ax1 = plt.subplot(211, xlabel='time [s]', ylabel='[kgcm^2]')
		ax1.plot(time, Ixx*1e4, label='Ixx(t)', lw=2, c='r') #Multiply by 10,000 to get correct unit
		ax1.set_title('Ixx of rocket during burn phase of %1.1f' % burnTime)

		ax2 = plt.subplot(212, xlabel='time [s]', ylabel='[kgcm^2]')
		ax2.plot(time, Iyy*1e4, label='I(t)', lw=2, c='b')  #Multiply by 10,000 to get correct unit
		ax2.set_title('Iyy and Izz(=Iyy) during burn phase of %1.1f' % burnTime)
		plt.show()
	...
	def from_file(initFile, dragFile, liftFile, momentsFile, path_to_file=''):
		...
		example: initFile.dot as initFile, myMotor.dot as motor and CFD files as drag.dot, lift.dot, moments.dot
							-stored in folder 'myRocket2' (in this case in the working folder)
				...
				myRocket = Rocket.from_file('initFile.dot', 'drag.dot', 'lift.dot', 'moments.dot',
											'myRocket2/')
